...
#define LONG_TICKS              (1500 / TICKS_INTERVAL)  // long press threshold
...
typedef enum {
	...
	BTN_STATE_RELEASE,      // released state waiting for timeout 已释放状态，等待超时
...
struct _Button {
	...
	uint8_t  repeat : 4;                // repeat counter (0-15) 重复触发计数（0–15）
	uint8_t  event : 4;                 // current event (0-15)
	uint8_t  state : 3;                 // state machine state (0-7)
	uint8_t  debounce_cnt : 3;          // debounce counter (0-7)
	uint8_t  active_level : 1;          // active GPIO level (0 or 1) 按键有效电平
	uint8_t  button_level : 1;          // current button level  保存最近一次读取到的 GPIO 电平值。
