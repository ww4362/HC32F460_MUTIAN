/********************************************************************************
    * 文件名称 ：IIC.c
    * 作     者：ZM
    * 版     本：V1.0
    * 编写日期 ：2024-01-30
    * 功     能：IIC驱动
*********************************************************************************/
 
#ifndef __IIC_H
#define __IIC_H

#include <stdint.h>
#include <main.h>
#include <stdio.h>



///* IO操作 */
//#define IIC_SCL(x)        do{ x ? \
//                              HAL_GPIO_WritePin(IIC2_SCL_GPIO_Port, IIC2_SCL_Pin, GPIO_PIN_SET) : \
//                              HAL_GPIO_WritePin(IIC2_SCL_GPIO_Port, IIC2_SCL_Pin, GPIO_PIN_RESET); \
//                          }while(0)       /* SCL */

//#define IIC_SDA(x)        do{ x ? \
//                              HAL_GPIO_WritePin(IIC2_SDA_GPIO_Port, IIC2_SDA_Pin, GPIO_PIN_SET) : \
//                              HAL_GPIO_WritePin(IIC2_SDA_GPIO_Port, IIC2_SDA_Pin, GPIO_PIN_RESET); \
//                          }while(0)       /* SDA */

//#define IIC_READ_SDA     HAL_GPIO_ReadPin(IIC2_SDA_GPIO_Port, IIC2_SDA_Pin) /* 读取SDA */
//													
//#define IIC_Delay     1


													
/* IIC所有操作函数 */
void iic_init(void);                        /* 初始化IIC的IO */
void iic_start(void);                       /* 发送IIC开始信号 */
void iic_restart(void);                     /* 发送IIC开始信号 */
void iic_stop(void);                        /* 发送IIC停止信号 */
void iic_ack(void);                         /* IIC发送ACK信号 */
void iic_nack(void);                        /* IIC不发送ACK信号 */
uint8_t iic_wait_ack(void);                 /* IIC等待ACK信号 */
void iic_send_byte(uint8_t txd);            /* IIC发送一个字节 */
uint8_t iic_read_byte(unsigned char ack);   /* IIC读取一个字节 */
void I2C_WriteRegByte(uint8_t SlaveAddress, uint8_t RegAddress, uint8_t ByteData); //通过I2C向设备寄存器写一个字节
uint8_t I2C_ReadRegByte(uint8_t SlaveAddress, uint8_t RegAddress); //通过I2C从设备寄存器读一个字节
void delay_us(uint32_t nus);
												


// 按照多项式 X^8+X^2+X^1+1 生成。
static const uint8_t crc8Table[256] =
{
	
};

uint8_t CRC8(uint8_t *data , uint8_t len);
void I2C_WriteRegByte_CRC8(uint8_t SlaveAddress, uint8_t RegAddress, uint8_t ByteData); //通过I2C向设备寄存器写一个字节(CRC校验)
uint16_t I2C_ReadReg2Byte_CRC8(uint8_t SlaveAddress, uint8_t RegAddress); //通过I2C从设备寄存器读两个字节
uint8_t I2C_ReadRegByte_CRC8(uint8_t SlaveAddress, uint8_t RegAddress); //通过I2C从设备寄存器读一个字节
#endif

